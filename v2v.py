...
class Simulation:
    def __init__(self, scheduler):
        ...
        '''
        for lane in vehicle_positions:
            for i, position in enumerate(vehicle_positions[lane]):
                #vehicle = Vehicle(id=(lane, i), position=position, speed=self.speed)
                vehicle_id = f'lane_{lane}_vehicle_{i}'
                vehicle = Vehicle(id=vehicle_id, position=position, speed=self.speed)
                print(f"Vehicle id {vehicle.id[1]} | Vehicle lane {vehicle.id[0]} | Vehicle positon : {vehicle.position} | Vehicle speed : {vehicle.speed}")
                self.scheduler.register_vehicle(vehicle)
                self.doca.add_vehicle(vehicle)
        '''

        for lane in vehicle_positions:
            for position in vehicle_positions[lane]:
                vehicle = Vehicle(id=(lane), position=position, speed=self.speed)
                self.doca.add_vehicle(vehicle)
                print(f"Vehicle lane {vehicle.id} | Vehicle positon : {vehicle.position} | Vehicle speed : {vehicle.speed}")
